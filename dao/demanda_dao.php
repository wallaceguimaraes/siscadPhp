...
class demanda_dao {
 ...
 function salvar(m_demanda $demanda) {

        try {
            
            $db = conexao::getConnection();
            
            $stmt = $db->prepare("INSERT INTO demanda (
                pessoa_id,demanda) 
                VALUES (:pessoa_id,:demanda);");
             
             $stmt->bindValue(":pessoa_id", $demanda->get__pessoa__id(), PDO::PARAM_INT);
             $stmt->bindValue(":demanda", $demanda->get__demanda(), PDO::PARAM_STR);
            ...
            return 0;
            $e = new Exception("Um erro ocorreu ao salvar a demanda. <br>" . $ex->getMessage());
            throw $e;
        }
    }
    ...
    function atualizar(m_demanda $demanda) {

        try {
            
            $db = conexao::getConnection();            
            if($demanda->get__status()==="CANCELADA"|$demanda->get__status()==="CONCLUÍDA"){
              $stmt = $db->prepare("
              UPDATE 
              ...
              UPDATE 
                ...
                where demanda_id=:demanda_id");
                                  ...
                                  $stmt->bindValue(":demanda_id", $demanda->get__demanda__id(), PDO::PARAM_INT);
                                  
                                  if($demanda->get__status()==="CONCLUÍDA"||$demanda->get__status()==="CANCELADA"){
                                    $stmt->bindValue(":status", $demanda->get__status(), PDO::PARAM_STR);
                                  }else{
                                    $stmt->bindValue(":demanda", $demanda->get__demanda(), PDO::PARAM_STR);
                                  }
                           ...
                           $stmt->execute();
            if($demanda->get__status()==="CANCELADA"){
              return 3;
            }else if($demanda->get__status()==="CONCLUÍDA"){
              return 4;
            }else{
              return 2;
            }
        ...
        public static function listar_demanda($filtro,$id) {
          ...
          if($filtro!==""){
              $consulta=" 
                       ...
                       FROM demanda d where ".$filtro." and d.ativo='SIM' order by d.data_cadastro desc";  
              
          }else{
          $consulta ="
                      SELECT d.*
                      FROM demanda d where d.ativo='SIM' order by d.data_cadastro desc ";
          }
          ...
          if($tamanho<20){
            ...
            $pessoa_id=$filtro[1];
                              ...
                              while($rows=$result->fetch(PDO::FETCH_ASSOC)){
                                ...
                                if($rows['status']==="REPROVADA"){$btnCancel="hidden"; $btnEditar="hidden"; $btnFinish="hidden";}
                                if($rows['status']==="ABERTO"){$btnCancel="hidden"; $btnEditar="hidden"; $btnFinish="hidden"; $btnObs="hidden";}
                                if($rows['status']==="ANDAMENTO"){$btnCancel=""; $btnEditar=""; $btnFinish="";$btnObs="";}  
                                  ...
                                  $l.= "<tr><td align-items=\"center\">".$rows['dem_70c']."</td>"
                                      ...
                                      ."<td style=\"width:12rem;\"align=\"center\">        
                                      <img ".$btnEditar." class=\"img-responsive img-rounded btn-editar bg-custom-5\" title=\"Editar\" id=\"btEditar\" onclick=\"selecionar(".$rows['demanda_id'].",'demanda.editar2',null,null,'pessoa_id')\" name=\"btEditar\" style=\"width:1.65rem;height:1.65rem;\" src=\"../ico/icons8-pencil-48.png\"></img>
                                      <img ".$btnObs." class=\"img-responsive img-rounded btn-editar bg-custom-5\" title=\"Observações\" id=\"btEditar\" onclick=\"selecionar(".$rows['demanda_id'].",'observacao.iniciar',null,null,'naoMuda','".$rows['status']."')\" name=\"btEditar\" style=\"width:1.65rem;height:1.65rem;\" src=\"../ico/icons8-goodnotes-48.png\"></img>
                                      <img  ".$btnFinish." class=\"img-responsive img-rounded btn-editar bg-custom-5\" title=\"Concluir demanda\" id=\"btEditar\" onclick=\"selecionar(".$rows['demanda_id'].",'demanda.concluir',null,'pessoa_id','naoMuda')\" name=\"btEditar\" style=\"width:1.65rem;height:1.65rem;\" src=\"../ico/icons8-task-completed-48.png\"></img>                          
                                      <img ".$btnVer." class=\"img-responsive img-rounded btn-excluir bg-custom-0\" title=\"Visualizar\" id=\"btVer\" onclick=\"selecionar(".$rows['demanda_id'].",'demanda.visualizar',null,null,'pessoa_id')\" style=\"width:1.65rem;height:1.65rem;\" src=\"../ico/icons8-eye-40.png\"></img>
                                      <img ".$btnCancel." class=\"img-responsive img-rounded btn-editar bg-custom-5\" title=\"Cancelar demanda\" id=\"btEditar\"  onclick=\"selecionar(".$rows['demanda_id'].",'demanda.cancelar',null,'pessoa_id','naoMuda')\" name=\"btEditar\" style=\"width:1.65rem;height:1.65rem;\" src=\"../ico/icons8-cancel-48.png\"></img>
